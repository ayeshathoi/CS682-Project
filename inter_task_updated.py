...
def replace_bn_with_lazy_bn(module):
    for name, child in module.named_children():
        if isinstance(child, torch.nn.BatchNorm2d):
            setattr(module, name, torch.nn.LazyBatchNorm2d().to(next(module.parameters()).device))
        else:
...
def main() -> None:
    ...
    # Fix stego_head input channels after SIH insertion
    stego_model.eval()
    with torch.no_grad():
        x_dummy, _ = next(iter(pet_train))
        x_dummy = x_dummy[:1].to(device)

        enc_out = stego_model.encoder(x_dummy)
        new_channels = enc_out.shape[1]

   # Patch stego_head input channels directly
    if isinstance(stego_model.stego_head, torch.nn.Conv2d):
        old = stego_model.stego_head

        new_head = torch.nn.Conv2d(
    ...
    # Initialize LazyBatchNorm layers before deepcopy
    stego_model.eval()
    with torch.no_grad():
        _ = stego_model.forward_stego(x_dummy)
    ...
    replace_bn_with_lazy_bn(recovered)

    # Initialize recovered model BatchNorm before PSNR evaluation
    recovered.eval()
    with torch.no_grad():
        x_dummy, _ = next(iter(pet_train))
        x_dummy = x_dummy[:1].to(device)
        _ = recovered.forward_secret(x_dummy)
