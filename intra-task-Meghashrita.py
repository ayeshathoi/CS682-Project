...
Usage in a notebook cell:
    %run dnn_steganography_colab.py
or just paste the whole file into a cell and run.

To change hyperparameters, edit the CONFIG dict below (or override after import).
"""
...
# ---------------------------------------------------------------------------
# VGG11 with batch-norm (works for 32x32 RGB input)
# ---------------------------------------------------------------------------
...
def get_fashion_mnist_loaders(batch_size, root, num_workers):
    ...
    train = datasets.FashionMNIST(root, train=True,  download=True, transform=tfm_train)
    test  = datasets.FashionMNIST(root, train=False, download=True, transform=tfm_test)
...
def run(cfg=None):
    """Run the full pipeline. Pass a dict to override CONFIG values, e.g.
        run({'secret_epochs': 5, 'stego_epochs': 5})    # quick smoke test
    """
    c = dict(CONFIG)
    if cfg:
        c.update(cfg)
    ...
    fm_train, fm_test = get_fashion_mnist_loaders(c['batch_size'], c['data_root'], c['num_workers'])
    ...
    cf_train, cf_test = get_cifar10_loaders(c['batch_size'], c['data_root'], c['num_workers'])
    ...
    load_extracted_into_model(recovered, {n: t.detach() for n, t in extracted.items()})
...
# Auto-run when executed as a script. In a notebook, prefer calling run(...) yourself.
